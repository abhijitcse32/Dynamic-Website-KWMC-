<?php
	...
	$sql="INSERT INTO slide (Image,SortID,CreateBy,CreateDate) VALUES ('$Image','$SortID','$CreateBy','$CreateDate')";
	if($flag)
	{
		$oResult=$oBasic->SqlQuery($sql);
		if($oResult->IsSuccess)
		{
			echo ("<script>window.alert('New Slide Show Image Upload Successfully!!!');</script>");
			echo ("<script>window.location='?Basic=Slide';</script>");
		}
		else
		{
...
elseif(isset($_POST['btnUpdate']))
{
	...
	for($i=0;$i<count($ID);$i++)
	{
		...
		$sql="UPDATE slide SET SortID='$SortIDN' WHERE ID=$IDN";
		$oResult=$oBasic->SqlQuery($sql);
	}
	if($oResult->IsSuccess)
	{
		echo ("<script>window.alert('New Slide Show Image Update Successfully!!!');</script>");
		echo ("<script>window.location='?Basic=Slide';</script>");
	}
}
...
if(isset($_GET['Delete']))
{
	...
	$sql="DELETE FROM slide WHERE ID=$Delete";
	$oBasic->SqlQuery($sql);
	$oResult=$oBasic->SqlQuery($sql);
	if($oResult->IsSuccess)
	{
		echo ("<script>window.alert('New Slide Show Image Delete Successfully!!!');</script>");
		echo ("<script>window.location='?Basic=Slide';</script>");
	}
}
?>
...
<head>
	...
	<script>
			function deletecheck(ID)
			{
				...
				{
					window.location="?Basic=Slide&Delete="+ID;
				}
			}
		</script>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Change Slide</h2>
            ...
            <div class="box-content row">
               ...
               <form style="margin-bottom:40px;" method="post" action="?Basic=Slide" enctype="multipart/form-data" id="registerform">
							<div class="form-group">
								<label class="admin_text" for="imageUpload">Image:</label>
                                <input type="file"  name="imageUpload" id="imageUpload" title="Upload Image size must be 941*310"/><b>Uploading Image Size must be  700*450 px</b>
                             </div>
